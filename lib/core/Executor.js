...
exports.Executor = class Executor {
  ...
  /**
   * Getter for the executor's version
   */
  version () {
    return VERSION;
  }
  ...
  _validateConnectorSupport (executionModule) {
    if (!this.connector.supportedExecution().includes(executionModule)) {
      throw(`Connector ${this.connector.name()} v${this.connector.version()} doesn't provide support for\
       '${executionModule}' execution on Executor v${this.version()}`);
    };
  };
};
