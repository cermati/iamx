...
const LIST_ACCESS_CONTEXT_EXTENSION_OPTIONS = 'listAccessContextExtensionOptions';
const VERSION     = '1.3.0';
...
exports.Executor = class Executor {
  ...
  /**
   * List down the available option values for certain field(s) in the
   * access context extension
   *
   * @param {Object} context
   */
  listAccessContextExtensionOptions(context) {
    this._validateConnectorSupport(LIST_ACCESS_CONTEXT_EXTENSION_OPTIONS);
    this._validateContext(context, LIST_ACCESS_CONTEXT_EXTENSION_OPTIONS);
    return this._execListAccessContextExtensionOptions(context);
  }
  ...
  /**
   * Querying list of available option values for certain field(s) in the
   * access context extension for the target platform.
   *
   * @param {Object} context  JSON object containing query parameters for the
   *                          access context extension options to be retrieved.
   */
  _execListAccessContextExtensionOptions(context) {
    let mContext = {...context};
    return this.connector.listAccessContextExtensionOptions(mContext);
  }
  ...
  _validateContext (context, execution) {
    ...
    switch (execution){
      ...
      case LIST_AVAILABLE_ACCESS_CONTEXT:
        ...
        break;
      case LIST_ACCESS_CONTEXT_EXTENSION_OPTIONS:
        this._validateObject(this.connector.listAccessContextExtensionOptionsContextFormat(), mContext);
        break;
      default:
